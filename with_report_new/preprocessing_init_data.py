...
def filtering_data(media_file, ga_file, history_file, condition_set):
    ...
    with st.spinner("데이터 가져오는 중..."):
        ...
        if commerce_or_not == "비커머스":
            cal_trans_metric_set = {'selected_trans_media': list_trans_media, 'selected_trans_ga': list_trans_ga}
        else:
            cal_trans_metric_set = {'selected_trans_media': "구매", 'selected_trans_ga': "구매"}

        return df_set, period_set, metric_set, cal_trans_metric_set
