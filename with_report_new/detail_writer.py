...
def writer(top_num, detail_df, sort_columns, criteria):
    ...
    if not pd.isna(total_sum):
        ...
        sentences["비용 분석"][f"가장 비용이 많이 사용된 {criteria}"] = (
            f" {max_index} / {max_value:,.0f} 원 "
        )
        sentences["비용 분석"][f"가장 비용이 적게 사용된 {criteria}"] = (
            f" {min_index} / {min_value:,.0f} 원 "
        )
    else:
        sentences["비용 분석"]["데이터 없음"] = f"해당 {criteria} 구분으로 활용된 데이터가 없습니다."

    # "유입 성과" section
    if not pd.isna(max_CPC):
        sentences["유입 성과"][f"가장 CPC가 높은 {criteria}"] = (
            f"{max_CPC_index} / {max_CPC:,.0f} 원"
        )
        sentences["유입 성과"][f"가장 CPC가 낮은 {criteria}"] = (
            f"{min_CPC_index} / {min_CPC:,.0f} 원"
        )
    else:
        sentences["유입 성과"]["데이터 없음"] = f"해당 {criteria} 구분으로 활용된 데이터가 없습니다."

    if not pd.isna(max_CTR):
        sentences["유입 성과"][f"가장 CTR이 높은 {criteria}"] = (
            f"{max_CTR_index} / {max_CTR:,.2f} %"
        )
        sentences["유입 성과"][f"가장 CTR이 낮은 {criteria}"] = (
            f"{min_CTR_index} / {min_CTR:,.2f} %"
        )
    else:
        sentences["유입 성과"]["CTR 데이터 없음"] = f"해당 {criteria} 구분으로 활용된 CTR 데이터가 없습니다."

    # "전환 성과" section based on the presence of "구매"
    if "구매" not in columns:
        if not pd.isna(max_CPA):
            sentences["전환 성과"][f"가장 CPA가 높은 {criteria}"] = (
                f"{max_CPA_index} / {max_CPA:,.0f} 원"
            )
            sentences["전환 성과"][f"가장 CPA가 낮은 {criteria}"] = (
                f"{min_CPA_index} / {min_CPA:,.0f} 원"
            )
        else:
            sentences["전환 성과"]["CPA 데이터 없음"] = f"해당 {criteria} 구분으로 활용된 CPA 데이터가 없습니다."

        if not pd.isna(max_COV):
            sentences["전환 성과"][f"가장 전환수가 높은 {criteria}"] = (
                f"{max_COV_index} / {max_COV:,.0f} 건"
            )
            sentences["전환 성과"][f"가장 전환수가 낮은 {criteria}"] = (
                f"{min_COV_index} / {min_COV:,.0f} 건"
            )
        else:
            sentences["전환 성과"]["전환수 데이터 없음"] = f"해당 {criteria} 구분으로 활용된 전환수 데이터가 없습니다."
    else:
        if not pd.isna(max_p):
            sentences["전환 성과"][f"가장 구매액이 높은 {criteria}"] = (
                f"{max_p_index} / {max_p:,.0f} 원"
            )
            sentences["전환 성과"][f"가장 구매액이 낮은 {criteria}"] = (
                f"{min_p_index} / {min_p:,.0f} 원"
            )
        else:
            sentences["전환 성과"]["구매액 데이터 없음"] = f"해당 {criteria} 구분으로 활용된 구매액 데이터가 없습니다."

        if not pd.isna(max_ROAS):
            sentences["전환 성과"][f"가장 ROAS가 높은 {criteria}"] = (
                f"{max_ROAS_index} / {max_ROAS:,.2f} %"
            )
            sentences["전환 성과"][f"가장 ROAS가 낮은 {criteria}"] = (
                f"{min_ROAS_index} / {min_ROAS:,.2f} %"
            )
        else:
            sentences["전환 성과"]["ROAS 데이터 없음"] = f"해당 {criteria} 구분으로 활용된 ROAS 데이터가 없습니다."
        if not pd.isna(max_COVR):
            sentences["전환 성과"][f"가장 전환율이 높은 {criteria}"] = (
                f"{max_COVR_index} / {max_COVR:,.2f} %"
            )
            sentences["전환 성과"][f"가장 전환율이 낮은 {criteria}"] = (
                f"{min_COVR_index} / {min_COVR:,.2f} %"
            )
        else:
            sentences["전환 성과"]["전환율 데이터 없음"] = f"해당 {criteria} 구분으로 활용된 전환율 데이터가 없습니다."


    # "GA 전환 성과" section based on the presence of "구매"
    if "GA_구매" not in columns:
        if not pd.isna(max_GACPA):
            sentences["GA 전환 성과"][f"가장 GA CPA가 높은 {criteria}"] = (
                f"{max_GACPA_index} / {max_GACPA:,.0f} 원"
            )
            sentences["GA 전환 성과"][f"가장 GA CPA가 낮은 {criteria}"] = (
                f"{min_GACPA_index} / {min_GACPA:,.0f} 원"
            )
        else:
            sentences["GA 전환 성과"]["GA CPA 데이터 없음"] = f"해당 {criteria} 구분으로 활용된 GA CPA 데이터가 없습니다."

        if not pd.isna(max_GACOV):
            sentences["GA 전환 성과"][f"가장 GA 전환수가 높은 {criteria}"] = (
                f"{max_GACOV_index} / {max_GACOV:,.0f} 건"
            )
            sentences["GA 전환 성과"][f"가장 GA 전환수가 낮은 {criteria}"] = (
                f"{min_GACOV_index} / {min_GACOV:,.0f} 건"
            )
        else:
            sentences["GA 전환 성과"]["GA 전환수 데이터 없음"] = f"해당 {criteria} 구분으로 활용된 GA 전환수 데이터가 없습니다."
    else:
        if not pd.isna(max_GAp):
            sentences["GA 전환 성과"][f"가장 GA 구매액이 높은 {criteria}"] = (
                f"{max_GAp_index} / {max_GAp:,.0f} 원"
            )
            sentences["GA 전환 성과"][f"가장 GA 구매액이 낮은 {criteria}"] = (
                f"{min_GAp_index} / {min_GAp:,.0f} 원"
            )
        else:
            sentences["GA 전환 성과"]["GA 구매액 데이터 없음"] = f"해당 {criteria} 구분으로 활용된 GA 구매액 데이터가 없습니다."

        if not pd.isna(max_ROAS):
            sentences["GA 전환 성과"][f"가장 GA ROAS가 높은 {criteria}"] = (
                f"{max_ROAS_index} / {max_ROAS:,.2f} %"
            )
            sentences["GA 전환 성과"][f"가장 GA ROAS가 낮은 {criteria}"] = (
                f"{min_ROAS_index} / {min_ROAS:,.2f} %"
            )
        else:
            sentences["GA 전환 성과"][f"GA ROAS 데이터 없음"] = f"해당 {criteria} 구분으로 활용된 GA ROAS 데이터가 없습니다."
        if not pd.isna(max_COVR):
            sentences["GA 전환 성과"][f"가장 GA 전환율이 높은 {criteria}"] = (
                f"{max_COVR_index} / {max_COVR:,.2f} %"
            )
            sentences["GA 전환 성과"][f"가장 GA 전환율이 낮은 {criteria}"] = (
                f"{min_COVR_index} / {min_COVR:,.2f} %"
            )
        else:
            sentences["GA 전환 성과"]["GA 전환율 데이터 없음"] = f"해당 {criteria} 구분으로 활용된 GA 전환율 데이터가 없습니다."
    ...
    with st.status(f"{criteria} 별 분석...") as status:
