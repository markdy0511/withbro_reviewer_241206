...
from with_report_new.grouping import grouped_media, grouped_ga
from with_report_new.reporting import report_media, report_ga, report_ga_add
from with_report_new.diff import comparing_df
from with_report_new.rounding import round_two_axis, round_multi_axis
...
def overview_df(media_df, ga_df, metric_set, cal_trans_metric_set, pr_trans_metric_set, group_period, condition_set, period_set):
    ...
    col_list = overview_df.columns.to_list()
    ...
    return rounded_overview_df, rounded_overview_df_filtered, col_list
...
def writer(rounded_overview_df, metric_col):
    ...
    if len(rounded_overview_df) == 1:
        ...
        for col in columns:
            ...
            if change_period[col] != 0 and change_period[col] != None and not(pd.isna(change_period[col])):
                if col in ["총비용", "구매액", "CPC", "CPA", "GA_CPA", "객단가", "GA_객단가", "GA_구매액"]:
                    sentences[col] = f"지난 기간 대비 {abs(change_period[col]):,.0f} 원 {change}. ({previous_period[col]:,.0f} 원 -> {current_period[col]:,.0f} 원, {change_rate_period[col]:,.2f} %)"
                elif col in ["CTR"]:
                    sentences[col] = f"지난 기간 대비 {abs(change_period[col]):,.2f} {change}. ({previous_period[col]:,.2f} -> {current_period[col]:,.2f}, {change_rate_period[col]:,.2f} %)"
                elif col in ["ROAS", "전환율","GA_ROAS", "GA_전환율"]:
                    sentences[col] = f"지난 기간 대비 {abs(change_period[col]):,.2f} % {change}. ({previous_period[col]:,.2f} % -> {current_period[col]:,.2f} %, {change_rate_period[col]:,.2f} %)"
                elif col in ["노출수", "클릭수"]:
                    change_rate = "증가" if change_rate_period[col] > 0 else "감소"
                    sentences[col] = f"지난 기간 대비 {abs(change_rate_period[col]):,.2f} % {change_rate}. ({previous_period[col]:,.2f} % -> {current_period[col]:,.2f} %, {change_rate_period[col]:,.2f} %)"
                else:
                    sentences[col] = f"지난 기간 대비 {abs(change_period[col]):,.0f} 건 {change}. ({previous_period[col]:,.0f} 건 -> {current_period[col]:,.0f} 건, {change_rate_period[col]:,.2f} %)"
            elif change_period[col] == None or pd.isna(change_period[col]):
                sentences[col] = f"{col} 데이터가 없음."
            else:
                sentences[col] = f"지난 기간과 동일."
    ...
    descript_pr = {"비용 변화":None, "유입 성과":None, "전환 성과":None,"GA 전환 성과":None, "총평":None}
    descript_pr["비용 변화"] = sentences["총비용"].split("하였음")[0].split("대비 ")[-1]
    
    inflow_summary_list = []
    for text in [sentences["노출수"], sentences["클릭수"], sentences["CTR"], sentences["CPC"]]:
        # 변화 추출 (증가 또는 감소)
        metric = text.split("은")[0].split("이")[0].strip()
        if "증가" in text:
            change = "증가"
        elif "감소" in text:
            change = "감소"
        ...
        # 요약 생성
        inflow_summary_list.append(f"{metric} {change}")

    # 결과 합치기
    inflow_summary = " / ".join(inflow_summary_list)
    descript_pr["유입 성과"] = inflow_summary
    ...
    if "구매" not in media_trans and "GA_구매" not in ga_trans:
        if current_period["전환수"] != 0 and current_period["전환수"] != None and not(pd.isna(current_period["전환수"])):
            media_trans_summary_list = []
            for text in [sentences[key] for key in media_trans]:
            # 변화 추출 (증가 또는 감소)
                metric = text.split("은")[0].split("이")[0].strip()
            if "증가" in text:
                change = "증가"
            ...
            # 요약 생성
            media_trans_summary_list.append(f"{metric} {change}")

            # 결과 합치기
            media_trans_summary = " / ".join(media_trans_summary_list)
            descript_pr["전환 성과"] = media_trans_summary
        ...
        if current_period["GA_전환수"] != 0 and current_period["GA_전환수"] != None and not(pd.isna(current_period["GA_전환수"])):
            ...
            # 요약 생성
            ga_trans_summary_list.append(f"{metric} {change}")

            # 결과 합치기
            ga_trans_summary = " / ".join(ga_trans_summary_list)
            descript_pr["GA 전환 성과"] = ga_trans_summary
            ...
            descript_pr["GA 전환 성과"] = "전환된 GA 성과가 없음."
            descript_raw["GA 전환 성과"] = "전환된 GA 성과가 없음."
        else:
            descript_pr["GA 전환 성과"] = "분석할 GA 전환 데이터가 없음."
