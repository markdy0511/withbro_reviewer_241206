...
def trend_days(reference_date, df_set, commerce_or_not, num_day):
    ...
    filtered_media_df = media_df[(media_df['일자'] >= reference - timedelta(days=num_day)) & (media_df['일자'] <= reference)]
    filtered_media_df['일자'] = filtered_media_df['일자'].dt.strftime('%Y-%m-%d')

    filtered_ga_df = ga_df[(ga_df['일자'] >= reference - timedelta(days=num_day)) & (ga_df['일자'] <= reference)]
